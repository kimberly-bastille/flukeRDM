/*******************************************************************************
 Script:       catch_per_trip_calibration_part1_refactored.do
 Purpose:      Estimates mean harvest-, discard- and catch-per-trip with
               ...
               catch_per_trip_calibration_part1.do. The original repeated one
               ~150-line MRIP prep block four times, a svy/postfile loop four
               times (plus two imputation variants), a results decoder four
               times and an export block three times; the projection script
               repeated Part A again. All of that is now defined once in
               catch_per_trip_programs.do, and this file is the calls.
               Behaviour is intended to be identical to the original, output
               for output; see the header of catch_per_trip_programs.do for
               the one output-identical deviation (D-1) and the preserved
               oddities.
 ...
 Pipeline:     Step 5a of model_wrapper.do, gated by the toggle catch_per_trip1.
               ...
               catch_per_trip_projection_part1_refactored.do, which runs the
               same Part A on a rolling window of MRIP data and has no Part B.
*******************************************************************************/

display "catch_per_trip_calibration_part1_refactored.do: estimating mean catch-per-trip and standard errors by state x wave x mode, imputing standard errors for single-PSU strata. This may take a while."
...
/* B.3: estimates by state, mode and wave. Note the domain string puts wave
   AFTER mode here, unlike Part A; wv2 is the wave string (deviation D-1 in
   catch_per_trip_programs.do: the original named it w2). */
tempfile domains ;
...
display "catch_per_trip_calibration_part1_refactored.do: finished. Wrote baseline_mrip_catch_processed.xlsx and the three mrip_catch_calib_*.dta files."
