################################################################################
################################################################################
# Script:       model_run_NC.R
# Purpose:      Runs the projection model for North Carolina under a saved
...
#               filename and regulation object name (nc rather than ma,
#               e.g. SFncFH_seas1_op), and the number of seasons the
#               state defines - 2 summer flounder seasons, 3 black sea bass, 2 scup. It also has three exists() branches.
...
#               L_W_Conversion.csv,
#               directed_trips_calibration_new_NC.feather,
#               proj_catch_draws_NC_<draw>.feather,
#               proj_year_calendar_adjustments_new_NC.csv,
#               base_outcomes_new_NC_<draw>_<mode>.CSV,
#               n_choice_occasions_new_NC_<mode>_<draw>.feather,
#               calibrated_model_stats_new.rds
# Outputs:      output_NC_<Run_Name>_<timestamp>.csv
...
set.seed(915)
message("model_run_NC.R: starting North Carolina projection for scenario '", Run_Name, "', 100 draws across 34 parallel workers. Expect a long run.")
